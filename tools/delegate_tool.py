...
_TOOLSET_LIST_STR = ", ".join(sorted(TOOLSETS))
...
def _build_child_agent(
    ...
    # ACP transport overrides from trusted delegation config/category/profile.
    ...
    try:
        ...
        # instead of disabling thinking for children. Capability/category
        # override still beats global delegation config.
        if override_reasoning_effort is not None and override_reasoning_effort != "":
            delegation_effort = override_reasoning_effort
        else:
            delegation_effort = delegation_cfg.get("reasoning_effort")
        if delegation_effort not in (None, "") or delegation_effort is False:
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    try:
        ...
        # Honor a capability-profile child_timeout_seconds override when set;
        # otherwise fall back to _get_child_timeout() (off by default —
        # result(timeout=None) blocks until the child finishes). Stuck-child
        # protection comes from the heartbeat staleness monitor.
