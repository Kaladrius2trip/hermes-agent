...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    agent_owned_tools = {"todo", "memory", "session_search", "delegate_task", "clarify"}
    memory_manager = getattr(agent, "_memory_manager", None)
    is_agent_owned_tool = function_name in agent_owned_tools or bool(
        memory_manager and memory_manager.has_tool(function_name)
    )
    ...
    if not is_agent_owned_tool and function_name not in valid_tool_names:
        return json.dumps({
            "error": (
    ...
    if function_name == "todo":
        ...
        if allowed_tool_names is not None:
            dispatch_kwargs["allowed_tool_names"] = allowed_tool_names
