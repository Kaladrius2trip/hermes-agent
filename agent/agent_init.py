...
def init_agent(
    ...
    # Inject memory provider tool schemas into the tool surface via upstream's
    # shared helper, passing the fork's ACL allow-set so a per-channel/role ACL
    # that excludes memory does not leak fact_store/etc. into the surface. The
    # helper also dedupes against names already registered via the plugin path
    # (duplicate function names 400 on providers enforcing unique names, e.g.
    # Xiaomi MiMo via Nous Portal) and honours the enabled_toolsets gate (#5544:
    # `platform_toolsets: telegram: []` must not leak memory tools).
    from agent.memory_manager import inject_memory_provider_tools as _inject_memory_provider_tools
    _inject_memory_provider_tools(agent, allowed_tool_names=allowed_tool_names)
    ...
    _acl_allowed_tool_names = set(allowed_tool_names) if allowed_tool_names is not None else None
