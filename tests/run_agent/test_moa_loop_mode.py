...
@pytest.mark.parametrize("provider", ["minimax-oauth", "qwen-oauth", "openrouter", "meridian"])
def test_moa_provider_backed_slot_survives_aux_resolution(monkeypatch, provider):
    ...
    NOTE: this covers the forward-the-resolved-endpoint path for providers that
    rely on _resolve_task_provider_model's provider-catalog preservation. If the
    catalog is unavailable, the fallback safety set must still include critical
    provider-backed routes such as OpenRouter/Meridian.
    """
    from agent import moa_loop
    from agent.auxiliary_client import _resolve_task_provider_model
