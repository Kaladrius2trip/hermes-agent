...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        (admin / user / unrestricted), slash commands they can run, and — when
        the Discord ACL store is active — the effective ACL policy.
        """
        ...
        if acl_store is not None:
            try:
                ...
                resolver = getattr(self, "_resolve_acl_policy_for_source", None)
                if callable(resolver):
                    acl_policy = resolver(source)
                    return legacy + "\n\nACL\n" + format_acl_policy(cast(Any, acl_policy))
            except Exception:
                pass
        return legacy
